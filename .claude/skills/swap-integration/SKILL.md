...
metadata:
  author: uniswap
  version: '1.3.0'
  vendored-from: Uniswap/uniswap-ai@3ddd8a9de93ef9201314c8759b5761c96ee7aebf
  vendored-path: packages/plugins/uniswap-trading/skills/swap-integration
  vendored-on: '2026-07-24'
  vendored-for: POO-1022 (Universal Funding epic)
---

<!--
  VENDORED THIRD-PARTY SKILL. Upstream: https://github.com/Uniswap/uniswap-ai
  (MIT, Copyright (c) 2026 Uniswap Labs — full text in ./LICENSE.upstream).
  Pinned at commit 3ddd8a9d, 2026-07-01. Refresh with `npx skills add Uniswap/uniswap-ai`
  and re-apply the two local edits below.

  LOCAL EDITS (only these — the body is upstream, unmodified):
    1. Removed `Task(subagent_type:swap-integration-expert)` from allowed-tools; that
       subagent ships with the upstream plugin and does not exist in this repo.
    2. Added this provenance block and the `vendored-*` metadata keys.

  SCOPE NOTE FOR THIS REPO — read before relying on it:
    This skill covers SAME-CHAIN swapping (Trading API /quote + /swap, Permit2 field
    rules, pre-broadcast validation, Universal Router, UniswapX). It does NOT cover
    Chained Actions — `POST /plan`, `PATCH /plan/:id`, `GET /plan/:id` — which is how
    Pool Party performs CROSS-CHAIN funding. For anything bridge-related the authority
    is `docs/_hackathon/02_BRIDGE_ARCHITECTURE.md`, not this file.

    Where this skill and our house skills disagree, OUR skills win: `wallet-operation-flow`
    (how an on-chain operation is orchestrated here), `server-data-access` (server actions
    and the secret boundary), and `frontend-security` (signing safety). In particular this
    skill's React examples call the Trading API from the client; in this repo the API key
    is server-only and every Uniswap call goes through a Server Action.
-->
