...
| `provisioning` | `NEXT_PUBLIC_FEATURE_PROVISIONING` | ⛔ off (dev on) | next | Pre-flight provisioning gate inside the op modals (invest/withdraw/collect/compound/move-range/close). **Not a route** — never route-guarded. Baseline is computed (`NODE_ENV === "development"`) so it demos in the local mock app and stays dark in prod/test; real wiring in POO-432. |
