...
| | |
| --- | --- |
| Contract | `TradingDaysHook` |
| Source | `src/TradingDaysHook.sol` |
| Mode | source |
...
| --- | --- | --- | --- |
| ⚠️ | I1 Conservation and solvency | skipped | TradingDaysHook's constructor takes 3 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I2 No undeclared extraction | skipped | TradingDaysHook's constructor takes 3 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I3 Exit liveness | skipped | TradingDaysHook's constructor takes 3 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
