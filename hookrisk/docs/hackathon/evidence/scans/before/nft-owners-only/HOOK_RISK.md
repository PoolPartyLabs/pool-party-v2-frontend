...
| | |
| --- | --- |
| Contract | `ERC721OwnershipHook` |
| Source | `src/ERC721OwnershipHook.sol` |
| Mode | source |
...
| --- | --- | --- | --- |
| ⚠️ | I1 Conservation and solvency | skipped | ERC721OwnershipHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I2 No undeclared extraction | skipped | ERC721OwnershipHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I3 Exit liveness | skipped | ERC721OwnershipHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
