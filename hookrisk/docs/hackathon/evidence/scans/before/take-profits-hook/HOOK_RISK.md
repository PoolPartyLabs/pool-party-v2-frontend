...
| --- | --- | --- | --- |
| ⚠️ | I1 Conservation and solvency | skipped | TakeProfitsHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I2 No undeclared extraction | skipped | TakeProfitsHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I3 Exit liveness | skipped | TakeProfitsHook's constructor takes 2 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
