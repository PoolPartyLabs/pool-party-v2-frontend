...
- 2 finding(s) at or above high: V2PairHook.afterSwap(address,PoolKey,IPoolManager.SwapParams,BalanceDelta,bytes) (src/V2PairHook.sol#196-207) is an IHooks callback (0xb47b2fb1) that never c...; V2PairHook (src/V2PairHook.sol#21-284) declares permission `beforeAddLiquidity` (bit 11, BEFORE_ADD_LIQUIDITY_FLAG) but provides no working `beforeAddLiquidi...
...
### 🟠 V2PairHook (src/V2PairHook.sol#21-284) declares permission `beforeAddLiquidity` (bit 11, BEFORE_ADD_LIQUIDITY_FLAG) but provides no working `beforeAddLiquidi...

`flag-implementation-divergence` · **high** · confidence **medium**

`src/V2PairHook.sol:21`

V2PairHook (src/V2PairHook.sol#21-284) declares permission `beforeAddLiquidity` (bit 11, BEFORE_ADD_LIQUIDITY_FLAG) but provides no working `beforeAddLiquidity` implementation. The PoolManager will call it on every matching pool operation and the call will revert, making the pool unusable for that operation.
...
| --- | --- | --- | --- |
| ⚠️ | I1 Conservation and solvency | skipped | V2PairHook's constructor takes 0 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I2 No undeclared extraction | skipped | V2PairHook's constructor takes 0 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
| ⚠️ | I3 Exit liveness | skipped | V2PairHook's constructor takes 0 argument(s); the harness supplies exactly one (the IPoolManager). Hooks with additional constructor arguments are not yet supported — see docs/INVARIANTS.md. |
