...
| Custom math | 2/5 | measured | Fee or weight formulas over pool state ᵃ |
...
| Price impacting behavior | 1/3 | measured | Charges a fixed, declared fee ᵃ |
...
- **Complexity**
  - hook-profile metrics: callbacksImplemented=2, callbacksDeclared=2, stateWritesInCallbacks=3, externalCallsInSwapPath=0, externalCallsInSwapPathThirdParty=0, internalFunctionsReachableFromCallbacks=13, usesReturnsDelta=true, hasOwnerOnlyFunctions=false
...
- **Custom math**
  - returns-delta is liquidity-side only (no swap delta); swaps still compare against the reference pool
...
| --- | --- | --- |
| Custom Curve or Non Standard Math | `returns-delta-permission` | hookrisk’s reading |
| Price Impacting Behavior | `priceImpactingBehavior >= 1 (is 1)`, `returns-delta-permission` | hookrisk’s reading |
...
| --- | --- |
| Fuzz sequences | 1285 |
| Swaps landed / compared / skipped | 13795 / 13795 / 0 |
| Swaps that reverted only with the hook | 0 |
| Positions opened / closed | 6460 / 6460 |
| Donations | 6835 |
| Price checks / monotonicity violations | 13795 / 0 |
| Exit failures | 0 |

The harness executed 13795 swap(s) (13795 compared against the reference pool, 0 skipped), opened 6460 and closed 6460 position(s), made 6835 donation(s) and ran 13795 price check(s) over 1285 sequence(s). An invariant with no relevant observations is reported inconclusive, not passed.
